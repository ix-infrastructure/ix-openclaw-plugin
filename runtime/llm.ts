 ...
 *
 * `--format llm` did not arrive all at once:
 *
 ...
 * with a rendered table, not an error. Asking `explain` for llm on 0.9.1
 * returns prose, successfully, and there is nothing to catch.
 *
...
import { runIx } from "../tools/base.ts";
 ...
 * is detected here and deferred, leaving each tool's own error envelope in
 * charge. No success record begins with `error code=`.
 */
 ...
 *
 * Every failure mode returns null: unsupported command, CLI too old, no CLI, a
 * non-zero exit, empty output, or an `error code=` record.
 */
export async function tryLlm(
  ...
  let out: string;
  try {
    out = await runIx([...args, "--format", "llm"], { cwd });
  } catch {
    return null;
  }

  // Scrubbed before it reaches the model. redactSecrets is idempotent and
  // order-free, so one pass over flat key=value lines cannot make the output
  // wrong.
